...
async function mapDatabase() {
  ...
  try {
    ...
    const outputPath = path.join(__dirname, '..', 'docs', 'DATABASE_SCHEMA.json');
    fs.writeFileSync(outputPath, JSON.stringify(schemaMap, null, 2));
    console.log(`\n✅ Full schema written to: ${outputPath}`);
    ...
    const mdPath = path.join(__dirname, '..', 'docs', 'DATABASE_SCHEMA.md');
    ...
    for (const [name, data] of sortedCollections) {
      console.log(`   ${name.padEnd(30)} ${data.count.toString().padStart(8)} docs`);
    }
    console.log('='.repeat(80));
